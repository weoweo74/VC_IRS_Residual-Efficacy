# Create shiny app in one single script together for both UI and SERVER
######################## HERE IS UI.R ######################################################
...
if(!require("shinyGlobe")) devtools::install_github("trestletech/shinyGlobe", force = TRUE)
...
if(!require("reshape")) install.packages("reshape")
if(!require("shinyRGL")) devtools::install_git("https://github.com/trestletech/shinyRGL")
if(!require("rthreejs")) devtools::install_github("bwlewis/rthreejs")
...
server <- function(input, output, session) {
  ...
  output$totLoss <- renderValueBox({
    valueBox("16 Countries", 
             " Tested with Pirimiphos-methyl",
             icon = icon("bank"), 
             color = 'purple') 
  })
    ...
    p <- plot_geo(attack_freq_country,height = 800) %>%
      add_trace(
        ...
        text = ~paste(paste("Country:",COUNTRY),
                      ...
                      paste("Mosquito Origin:", Funder),sep = "<br />"), 
        locations = ~CODE,
        marker = list(line = l), 
        hoverinfo = "text") %>%
      colorbar(title = '', 
  ...
  output$plot3 <- renderPlotly({
    ...
    p3 <- plot_ly(death_country, 
                  ...
                  insidetextfont = list(color = '#FFFFFF'), 
                  hoverinfo = 'text',
                  text = ~paste('Total Fatalities:', DEATH),
                  showlegend = FALSE) %>%
      layout(margin = m,
             xaxis = list(showgrid = FALSE, zeroline = FALSE, showticklabels = FALSE),
             yaxis = list(showgrid = FALSE, zeroline = FALSE, showticklabels = FALSE)) %>%
  ...
  output$plot5 <- renderPlotly({
    ...
    p5 <- plot_ly(attack_category) %>%
      add_trace(x = ~iyear, 
                ...
                line = list(width = 2), 
                hoverinfo = "text", 
                text = ~paste(paste("Total Malaria:", ones), 
                              attack_type,
                              sep = "<br />"), 
  ...
  output$plot6 <- renderPlotly({
    ...
    levels(death_year$attack_type) <- c("IRS_pop_protected","Sprayed_Structures", "IRS_Coverage", 
                                        "Children_U5yrs_protected","IRS_pop_Not_protected", 
                                        "PregWomen_U5yrs_sleeping_underNet_IRS_protected", 
                                        "Spray_Structures_found", "UnIRS_pop_protected", "Unknown")
    attack_category2 <- cast(death_year,iyear ~ attack_type, sum)
    
    p6 <- plot_ly(attack_category2, 
                  x = ~iyear, 
                  y = ~IRS_pop_protected, 
                  type = 'bar', 
                  name = 'IRS_pop_protected',
                  text = ~paste(IRS_pop_protected*IRS_pop_protected,":IRS_pop_protected"),
                  hoverinfo = 'text') %>%
      add_trace(y = ~Sprayed_Structures, 
                name = 'Sprayed_Structures',
                text = ~paste(Sprayed_Structures*Sprayed_Structures,":Sprayed_Structures")) %>%
      add_trace(y = ~IRS_Coverage, 
                name = 'IRS_Coverage',
                text = ~paste(IRS_Coverage*IRS_Coverage,":IRS_Coverage")) %>%
      add_trace(y = ~Children_U5yrs_protected, 
                name = 'Children_U5yrs_protected',
                text = ~paste(Children_U5yrs_protected*Children_U5yrs_protected,":Children_U5yrs_protected")) %>%
      add_trace(y = ~IRS_pop_Not_protected, 
                name = 'IRS_pop_Not_protected',
                text = ~paste(IRS_pop_Not_protected*IRS_pop_Not_protected,":IRS_pop_Not_protected")) %>%
      add_trace(y = ~PregWomen_U5yrs_sleeping_underNet_IRS_protected, 
                name = 'PregWomen_U5yrs_sleeping_underNet_IRS_protected',
                text = ~paste(PregWomen_U5yrs_sleeping_underNet_IRS_protected*PregWomen_U5yrs_sleeping_underNet_IRS_protected,":PregWomen_U5yrs_sleeping_underNet_IRS_protected")) %>%
      add_trace(y = ~Spray_Structures_found, 
                name = 'Spray_Structures_found',
                text = ~paste(Spray_Structures_found*Spray_Structures_found,":Spray_Structures_found")) %>%
      add_trace(y = ~UnIRS_pop_protected, 
                name = 'UnIRS_pop_protected',
                text = ~paste(UnIRS_pop_protected*UnIRS_pop_protected,":UnIRS_pop_protected")) %>%
      add_trace(y = ~Unknown, 
                name = 'Unknown',
                text = ~paste(Unknown*Unknown,":Unknown") ) %>%
      layout(yaxis = list(title = ' SQRT(Case Mgt)',
                          range = c(0, 450)), 
             xaxis = list(title=""), 
             barmode = 'stack',
             legend = list(x = 0.06, y = 0.98))  %>% 
  ...
  output$plot7 <- renderPlotly({
    ...
    p7 <- plot_ly(country_type_attack, 
                  labels = ~attack_type, 
                  values = ~ones, 
                  type = 'pie',
                  hole = 0.8,
  ...
  output$plot8 <- renderPlotly({
    ...
    p8 <- plot_ly(country_type_target, 
                  labels = ~target_type, 
                  values = ~ones, 
                  type = 'pie',
                  hole = 0.8,
  ...
  output$plot9 <- renderPlotly({
    ...
    p9 <- plot_ly(country_type_weapon, 
                  labels = ~weapon_type, 
                  values = ~ones, 
                  type = 'pie',
                  hole = 0.8,
                  textinfo = 'none',
                  insidetextfont = list(color = '#FFFFFF'),
                  hoverinfo = 'text',text = ~paste('Total Malaria:', ones),
                  showlegend = T) %>%
      layout(margin = m,
             xaxis = list(showgrid = FALSE, zeroline = FALSE, showticklabels = FALSE),
             yaxis = list(showgrid = FALSE, zeroline = FALSE, showticklabels = FALSE),
  ...
  output$plot11 <- renderPlotly({
    ...
    p11 <- plot_ly(
      ...
      showlegend = FALSE) %>%
      layout(title = "Malaria Intervention", 
  ...
  output$plot12 <- renderPlotly({
    ...
    p12 <- plot_ly(
      ...
      color = c("Suicide","Success","Multiple","Weapon Type", "Attack Type","Fatalities",
                "Target Type"),
      showlegend = FALSE) %>%
      layout(title = "Importance of Paramters",
             xaxis = list(categoryarray = c("Suicide", "Success","Multiple","Weapon",
                                            "Attack","Fatalities","Target"), 
                          categoryorder = "array"),
  ...
  output$groupnameTbl <- DT::renderDataTable({
    ...
    dat1 <- datatable(country_group_name, 
                      options = list(initComplete = JS("function(settings, json) {
                                                        $(this.api().table().header()).css({
                                                        'background-color': '#fff',
                                                        'color': '#444'
  ...
  output$regionlist <- renderUI({
    ...
    selectInput("byregion", 
                label = "Region:", 
                choices = c(Choose = '', as.character(region_name)), 
  ...
  output$countrylist <- renderUI({
    ...
    selectInput("bycountry", 
                label = "Country:", 
                choices = c(Choose = '', as.character(country_name)), 
  ...
  output$totAttacks_country <- renderValueBox({
    ...
    valueBox(country_no_attack,
             "Total IRS Population",
             icon = icon("globe"), 
             color = 'red') 
  })
  ...
  output$totloss_country <- renderValueBox({
    ...
    valueBox(paste("$",country_finance_loss,sep = ""),
  ...
  output$totlife_country <- renderValueBox({
    ...
    valueBox(country_life_loss,
             "Total ITN Distributed",
             icon = icon("user"), 
             color = 'blue') 
  })
  ...
  output$country_map <- renderLeaflet({
    ...
    m1 <- leaflet(country_data) %>% 
      addProviderTiles("CartoDB.DarkMatterNoLabels", 
                       options = providerTileOptions(opacity = 0.7)) 
    
    pal <- colorNumeric(palette = "YlOrRd", 
                        domain = country_data$sqrt)
    m1 <- m1 %>% 
      addCircleMarkers(radius = 2,
                       color = ~pal(sqrt),
                       popup = ~paste(paste0("Fatalities: ", nkill),
                                      paste0("City: ", city),
                                      paste0("Date: ", date),
                                      sep = '<br />'),
                       opacity = 0.9)
